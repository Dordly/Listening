...
- (void)bindAnchorTracksData:(NSDictionary *)tracksDictionary {
    ...
    [self.coverIV setImageWithURL:iconURL placeholderImage:[UIImage imageNamed:@"album_cover_bg"]];
...
- (UIButton *)downloadBtn {
    if(_downloadBtn == nil) {
        ...
        [_downloadBtn bk_addEventHandler:^(id sender) {
            ...
            if ([self.delegate respondsToSelector:@selector(musicDetailCellDidClickDownloadButton:)]) {
                [self.delegate musicDetailCellDidClickDownloadButton:self.indexPath];
            }
