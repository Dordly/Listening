...
/**  获取推荐内容数组 */
//- (NSArray *)categoryContentsForModel {
//    return return self.model.categoryContents.list;
//}
...
/**  通过分组数, 获取moduleType */
- (NSInteger)moduleTypeForSection:(NSInteger)section {
    return self.model.categoryContents.list[section].moduleType;
}
...
- (NSURL *)coverURLForIndexPath:(NSIndexPath *)indexPath {
    ...
    } else if ([self moduleTypeForSection:indexPath.section] == 4){
        path = self.model.categoryContents.list[indexPath.section].list[indexPath.row].coverPathSmall;
    } else {
...
/**  通过分组数和行数(IndexPath), 获取key */
- (NSString *)keyForIndexPath:(NSIndexPath *)indexPath {
    return self.model.categoryContents.list[indexPath.section].list[indexPath.row].key;
}
/**  通过分组数和行数(IndexPath), 获取content */
- (NSString *)contentTypeForIndexPath:(NSIndexPath *)indexPath {
    if ([self moduleTypeForSection:indexPath.section] == 4) {
        return self.model.categoryContents.list[indexPath.section].list[indexPath.row].footnote;
    } else {
        return self.model.categoryContents.list[indexPath.section].list[indexPath.row].contentType;
    }
}
/**  通过分组数和行数(IndexPath), 获取副标题 */
- (NSString *)subTitleForIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 || [self moduleTypeForSection:indexPath.section] == 4) {
...
- (NSInteger)albumIdForIndexPath:(NSIndexPath *)indexPath {
    if ([self moduleTypeForSection:indexPath.section] == 4) {
        return self.model.categoryContents.list[indexPath.section].list[indexPath.row].specialId;
    } else {
