...
//        _btn1.backgroundColor = [UIColor redColor];
        [_btn1 bk_addEventHandler:^(id sender) {
            ...
            if ([self.delegate respondsToSelector:@selector(categoryCommonCellsubButtonDidClick:)]) {
                [self.delegate categoryCommonCellsubButtonDidClick:_index-1];
            }
        } forControlEvents:UIControlEventTouchDown];
    }
    return _btn1;
...
//        _btn2.backgroundColor = [UIColor yellowColor];
        [_btn2 bk_addEventHandler:^(id sender) {
            ...
            if ([self.delegate respondsToSelector:@selector(categoryCommonCellsubButtonDidClick:)]) {
                [self.delegate categoryCommonCellsubButtonDidClick:_index];
            }
        } forControlEvents:UIControlEventTouchDown];
    }
    return _btn2;
