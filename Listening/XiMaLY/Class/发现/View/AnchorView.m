...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self.attractBtn bk_addEventHandler:^(UIButton* sender) {
            ...
            if ([self.delegate respondsToSelector:@selector(anchorViewDidClickAttractButtom:index:isAttracted:)]) {
                [self.delegate anchorViewDidClickAttractButtom:_indexPath index:self.tag isAttracted:sender.selected];
            }
...
- (UIButton *)anchorBtn {
    if (!_anchorBtn) {
        ...
        [_anchorBtn bk_addEventHandler:^(UIButton * sender) {
            
            if ([self.delegate respondsToSelector:@selector(anchorViewDidClickButtom:index:)]) {
                [self.delegate anchorViewDidClickButtom:_indexPath index:self.tag];
            }
        } forControlEvents:UIControlEventTouchUpInside];
